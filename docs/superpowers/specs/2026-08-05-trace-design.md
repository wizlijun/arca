...
> 日期：2026-08-05 · 状态：**已融入 spec，本文降级为设计过程记录**
>
> **本文不再是真相源。**结论已融入 `docs/2026-08-03-arca-spec.md` §3.3（v1.2），
> 字节契约在 `FORMAT.md` §10，命令与错误码契约在 `PROTOCOL.md` §5.1/§7——
> **三者才是权威**。本文保留的是「为什么这样选」的推导过程：被否掉的方案、
> 与 git trace2 的两处刻意偏离、可读性输给确定性的那次取舍。
> 二者若出现分歧，以 spec 为准（I10：格式先于代码，规范先于设计稿）。
>
