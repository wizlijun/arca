...
| 组件 | 许可证 | 理由 |
| --- | --- | --- |
| **全部组件**（含 arcad） | **Apache-2.0** | 单一许可证，最大化嵌入与生态采用；自带专利授权与商标条款 |
| 商标 "arca"（定名后） | 维护者实体持有 | 开源不放商标，治理的最后防线（Apache-2.0 §6 已明确不授予商标许可） |

> **v1.2 修订**：早期草案对服务端 arcad 采用 AGPL-3.0（防云厂商白嫖托管、
> 保留 Nextcloud 式商业选项），对库采用 Apache-2.0 OR MIT。现统一为 Apache-2.0——
> 放弃以许可证作为商业护栏，改由商标与协调面服务承担该角色。
