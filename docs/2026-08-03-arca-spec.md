...
| **I5** | **绝不猜测**：状态模糊 → 停下并可诊断（诊断轨迹见 §3.3），而不是尽力恢复 | §4 |
...
- **plumbing**：`arca ls --json`（清单/状态枚举）、`arca cat <hash>`（按哈希取内容）、  
  ...
  `arca trace last --json`（最近一次失败的决策轨迹，§3.3）。  
...
### 3.3 可诊断性：trace（继承 git 的 trace2）

I5 承诺「状态模糊 → 停下并**可诊断**」。「可诊断」这三个字需要一个机制来兑现，  
否则它只是一句漂亮话。这个机制就是 trace——**结构化的决策轨迹**，不是按级别打的文本日志。

驱动需求来自 agent：一次 `arca push` 失败，真正要回答的是「调和引擎对这 3000 个文件  
分别决定了什么、在第几个上停的、当时本地/远端/基线三方各是什么状态」。  
自由文本日志给 agent 的是一堆要用正则去捞的字符串；trace 给的是可精确匹配的结构。

字节级契约见 `FORMAT.md` §10，命令与错误码契约见 `PROTOCOL.md` §5.1/§7；  
本节只讲设计决定与它们的理由。

#### trace 不是 journal——三个概念必须严格分开

否则迟早有人往 journal 里塞调试信息，journal 就废了。

| | 位置 | 记什么 | 生命周期 | 损坏时 |
| --- | --- | --- | --- | --- |
| **journal**（§4.2） | hub `.arca/journal/` | 数据事实 + actor（I8） | 永久，是真相 | 中间行损坏 → **失败** |
| **`.txn`**（§4.2） | hub `.arca/locks/` | 不可逆点的前滚/回滚意图 | 事务完成即清 | 前滚或回滚，绝不猜测（I5） |
| **trace** | 客户端 `<state>/trace/` | 一次会话的决策轨迹 | 最近 N 次失败 | 坏行**跳过并计数** |

**两条相反的损坏纪律是刻意的**：journal 是真相，读错一行等于伪造历史，必须失败；  
trace 是事故现场的线索，为一行坏数据丢掉其余几千条线索是荒谬的。  
这是全设计里唯一需要反直觉记住的一条。

`.txn` 保持自己的格式与 fsync 保证、是崩溃恢复的权威依据；每次 `.txn` 写入**同时**  
向 trace 发一条镜像事件，可丢。于是 agent 读 trace 能看到完整故事，  
而崩溃恢复的正确性不依赖 trace。

#### 命名与结构对齐 git trace2

`sid`（层次化会话标识）· `event` · `t_abs` · `region_enter`/`region_leave` ·  
环境变量 `ARCA_TRACE_EVENT` · `arca bugreport`——全部借用 `GIT_TRACE2` 的既有词汇  
（§14）。**层次化 sid 是白捡的收益**：子进程继承父 sid 并追加自己的一段，  
于是 `arca sync` 与它调起的 `fetch`/`push`（§3.2 的 porcelain/plumbing 分层）  
天然串成一棵树，agent 拿到根 sid 就能捞出整棵。

#### 让 agent 好用的两个决定

1. **`event` 是封闭枚举**，进 `FORMAT.md` §10.3，受 I10 约束。agent 对它做精确匹配，  
   而不是正则捞字符串。读侧对未知事件名**原样透传**——新版本写出的 trace，旧版本必须能读。
2. **错误按处置分类**，而不只是给个错误码。`class` ∈ `retryable`（退避重试）/  
   `needs_human`（**停下**，I5）/ `protocol`（走结构化冲突流程）/ `bug`（提 issue）。  
   **agent 只看 `class` 就知道该做什么，无需理解 `code` 的语义**——`code` 服务于人和长期兼容，  
   `class` 服务于自动化。码表见 `PROTOCOL.md` §7。

载荷字段只允许标量、不允许嵌套，且按键名字节序输出（同 manifest 的确定性序列化纪律，  
§4.4.1）。前者让 `jq` 无需展开嵌套、也防止有人把整个结构体塞进 trace 让 schema 失控；  
后者让归并多个会话后重新序列化仍逐字节可复现，arca-conformance 才能比对第三方实现。

#### sans-io 约束下 core 怎么产出 trace

arca-core 不能写日志（§11.3：无 IO、无运行时）。**core 只产出载荷，信封由 sink 补齐**：  
入口函数接一个 `TraceSink`，内部 push 结构化记录；`t_abs` 取自调用方注入的时钟——  
这不破坏 sans-io，因为 §11.2 的确定性模拟测试本来就要注入模拟时钟。

三个 sink 随 `arca-format` 出厂：`NullSink`（零成本丢弃）、`VecSink`（测试留存）、  
`RingSink`（生产环形缓冲）。`VecSink` 给正确性基础设施带来的增益见 §11.2。

#### 落盘：成功即丢弃

进程内环形缓冲始终记录；**正常退出不写任何文件**（§3.2 的 Rule of Silence），  
非零退出或 panic 才把整个环 flush 到 `<state>/trace/`。  
这个策略顺带把日志轮转这个老大难消掉了——平时不产生文件，自然不需要轮转；  
保留由下一次进程启动时顺手 GC（客户端零常驻，没有别人能做这事，§3.1）。

**trace 刻意不 fsync。**真正不能丢的是 `.txn` 与 journal，二者各自已有 fsync 保证；  
trace 丢了，事实仍完整存在于它们之中。给每条 trace 上 fsync 会让一次万文件同步  
多出几万次 fsync，为一个次要的东西付出一个数量级的代价，还会诱使人们把 trace 当真相依赖。  
环形缓冲挤掉的条数必须如实落进文件——沉默地截断线索，读的人会误以为「前面什么都没发生」，  
这与 I5 同源。

#### 查询面与审计闭环

agent 不该去 grep 文件，该有 plumbing（§3.2 分层）：

| 命令 | 层 | 用途 |
| --- | --- | --- |
| `arca trace list` / `show <sid>` / `last` | plumbing | 列出、导出、取最近一次失败——**agent 九成场景只需要 `last`** |
| `arca doctor` | porcelain | 检查当前健康度（对应 `git fsck`），附最近失败的 sid |
| `arca bugreport` | porcelain | 打包 trace + doctor + 版本环境（对应 `git bugreport`） |

**集中的范围是单机，不自动回传**：同一台机器上所有 arca 进程写同一个 state 目录，  
hub 侧 arcad 自有一份。同步失败往往就是网络坏了，自动回传在最需要它的时刻恰好不可用；  
跨机器合并走显式的 `arca bugreport`（外发时须脱敏路径）。

两侧靠 `sid` 关联：客户端提交时把 `sid` 放进请求头，arcad 记入 journal 事件的  
`actor.session`（§4.1）——I8「每个事件可归因」由此闭环到具体是哪台机器的哪次会话。
...
| `vault_diagnose` | 取最近一次失败会话的结构化 trace（§3.3） | 错误带 `class`，agent 据此决定重试 / 停下 / 报告，无需理解 `code` 语义 |
...
| **决策序列断言** | arca-core + trace（§3.3） | `VecSink` 留存 core 产出的决策记录，测试直接断言**推理路径**而非只断言最终状态——proptest 缩小出反例时，看到的不是「结果不对」，而是引擎在第几步选错了哪个动作；I3 由此可写成「trace 中不出现任何销毁性动作」 |
...
| 里程碑 | 交付 | 验收标准（可演示、可度量） |
| --- | --- | --- |
| **M0 格式与核心** | FORMAT.md v1 · arca-format · arca-chunk · fsck · **trace 事件 schema 与 sink**（§3.3；`event` 是封闭枚举、受 I10 约束，必须在任何实现开始发事件之前定死） | coreutils 恢复演示进 CI；fuzz 72h 无 panic；golden vectors 就绪（含 trace 的合法与损坏两份，锁住「坏行跳过并计数」这条与 journal 相反的纪律） |
| **M1 单机纳管 + file:// 同步** | CLI：setup / adopt / add / verify / history / restore / gc / bundle · plumbing 层（ls/cat/resolve/state dump/trace，§3.2） · **trace 发射点与失败落盘**（§3.3） · **file:// 直连同步**（无任何 daemon 的完整闭环，§3.1） · arca-git（`.gitarca` 注册表 + `.gitignore` 反选块 + 清单 + pre-push 钩子 + 追踪冲突检测） | 在一个真实 Obsidian 库上 `arca adopt`：**文件原地不动、git status 干净、清单进 git、后续提交不再增长**（存量历史瘦身需另行重写历史，见下注）；1 万张照片 2 分钟内去重归档 + 全量校验 |
| **M2 arcad 与手动同步** | arcad · CAS · tombstone · journal+longpoll · 多卷映射 · server/client 角色 · **多 hub 独立故障域** · 手动 `pull/push/status`（无客户端 daemon，§3.1） · `sid` 进协议头与 journal 的 `actor.session` 闭环 · `arca bugreport`（§3.3） | 模拟测试：随机交错 + 崩溃注入下收敛且零销毁；两机改名/删除/冲突全场景（纯手动命令完成）；拔盘演练：卷离线呈现为数据集离线而非空库（I11） |
...
| **M6 agent 接口** | arca-mcp · agent 令牌 · checkpoint · `vault_diagnose`（§10.1） | 两个并发 agent 在同一库上无丢失更新地协作（§10.2 场景自动化）；agent 遇失败能只凭 `class` 决定重试或停下，无需人工介入判读 |
...
- git trace2（诊断轨迹的命名与事件模型来源，§3.3） — https://git-scm.com/docs/api-trace2
...
| v1.2 | 补上 I5「可诊断」的兑现机制：新增 §3.3 可诊断性 trace（继承 git trace2）——结构化决策轨迹、`event` 封闭枚举、错误按处置四分类、成功即丢弃的落盘策略、`sid` 闭环回 I8；随之增补 §3.2 plumbing 清单、§10.1 `vault_diagnose`、§11.2 决策序列断言、§12.3 各里程碑落位。字节契约见 FORMAT.md §10，命令与码表见 PROTOCOL.md §5.1/§7 |

---

*v1.2 · 2026-08-05。继承关系：凡标注"继承 §n"处，指 DESIGN.CN.md 第一部分对应章节的设计，  
其正确性论证与边界讨论以该文为准。*