...
| `400` | `request.wait_invalid` | `GET .../changes` 的 `wait` 语法根本不是非负整数（评审 Minor 项）——与"数值合法只是超出 `u64` 表示范围"不同性质：后者按本节「`wait` 钳制」一段的承诺静默取 90，不报错；`wait`/`limit` 两个查询参数都不用 `axum::extract::Query` 直接反序列化成整数类型，避免语法/范围问题在进入 handler 之前就被 axum 兜底成一个没有 `code` 字段的纯文本 400 |
