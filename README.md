...
> 没有第二份副本。**`git clean -Xdf`（大写 `X`，只清理被忽略的文件）同样会删掉受管
> 二进制**——它常被当成"只清理构建产物"的安全肌肉记忆，但判据仍然是"被忽略"，受管
> 二进制照样中招；`-x`/`-X` 在这件事上没有区别，只有小写 `-x`（同时清理未追踪与被
> 忽略的文件）与 `-X` 的差异体现在其它未追踪文件上。缓解措施：`arca doctor` 会检出
> "本地存在但 hub 尚无副本"的文件并显著告警——在跑 `git clean` 之类的清场命令前，
> 养成先看一眼它的输出的习惯。

> **上手：[docs/USAGE.md](docs/USAGE.md)** —— 按主次排列的功能走查，每条命令与输出都实机跑过。
