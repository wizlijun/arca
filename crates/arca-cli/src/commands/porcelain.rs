...
fn print_gc_report(
    ...
    if report.executed {
        ...
        // 「有 .meta 但磁盘上没有 .data」的那些条目也在上面这句里，可它们
        // 与普通销毁**根本不是一回事**：什么都没回收，被删掉的只是一条
        // 记录。放着不说，用户唯一能察觉的线索就是清单里那个 0 字节。
        //
        // 这个状态是**歧义**的，两种成因都可能：gc 自己上次销毁到一半崩了
        // （先 `.data` 后 `.meta`，中间态合法，补删是自愈，见 `gc::plan`）；
        // 或者那份内容以别的方式丢了，而这条 `.meta` 是「丢的是哪个文件」
        // 的最后一条线索。gc 选自愈是对的——把它当 blocker 会让任何一次
        // gc 崩溃永久堵死后续所有 gc——但**选了之后要说出来**，
        // 让人有机会认出第二种成因（I5 的诊断面：不猜，但也不闷声）。
        let 无内容 = report.destroyed.iter().filter(|c| !c.data_present).count();
        if 无内容 > 0 {
            eprintln!(
                "其中 {无内容} 条在磁盘上**已经没有内容**，只删掉了记录本身、\
                 没有回收任何空间。通常是上一次 gc 销毁到一半被打断留下的残留，\
                 本次顺手补齐；但如果你并不记得有过中断的 gc，那更可能是这些\
                 内容以别的方式丢了——以上清单里的原路径就是唯一的线索，\
                 值得对照别的设备或备份核一遍。"
            );
        }
    } else {
