...
//!
//! TODO(M5+)：**§6.3 第 10 条「清单与实体一致性」这条噩梦路径测试尚未实现**。
//! 它要断言的是：`git checkout` 到旧提交之后，arca 按**当时那份清单**还原出
//! 对应版本的二进制，而不是留下当前版本。本模块提供了比对所需的全部素材
//! （路径/哈希/大小），缺的是那条端到端测试本身——它要同时驱动 git 与
//! `arca sync`，属 `arca-cli` 的集成测试范围，不是本模块能自己写的。
